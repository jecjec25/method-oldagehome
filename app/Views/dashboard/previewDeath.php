...
<body>
<form method="get" action="<?= base_url('getReportsDeath/' .$search. '/' . $tosearch) ?>">
    ...
    <div class="size">
        ...
        <h4 style="text-align: center; font-size:13px;">LIST OF ELDERS (DECEASED)</h4>
        <p>Date: <?= date('F j, Y', strtotime($currentDate)) ?></p>
        <p>Reporting Period: <?= date('F j, Y', strtotime($search)) ?> - <?= date('F j, Y', strtotime($tosearch ))?></p>
